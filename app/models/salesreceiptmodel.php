...
class SalesReceiptModel extends AbstractModel
{
    use filter;

    protected static $tableName = 'sales_receipt';
    ...
    public static function getAll_receiptssales($id = false , $bill_id = false)
    {
        $where = '';
        $receipt_price = '';
        ...
        if ($bill_id != false && is_numeric($bill_id) ){
            $receipt_price = ',(SELECT SUM(sales_receipt.receipt_price) FROM sales_receipt WHERE sales_receipt.bill_id = '.$receipt_id.' ) as oldreceipt';
        }

        return parent::getAll('SELECT sales_receipt.* ,  users.username as username , 
        (SELECT SUM(sales_orders.order_price) FROM sales_orders WHERE sales_orders.sales_bill_id = sales_receipt.bill_id ) as finalorderprice 
        '.$receipt_price.'
        FROM `sales_receipt` 
        JOIN users ON users.user_id = sales_receipt.user_id '.$where);
    }
